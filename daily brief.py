...
def summarise_article(article: dict) -> str:
    # 1. Try full-text scrape
    content = scrape_full_text(article["link"])
    # 2. Fall back to RSS summary
    if not content:
...
def build_brief(articles_by_category: dict) -> dict:
    brief = {}
...
def render_html(brief: dict) -> str:
    ...
    for category, articles in brief.items():
        ...
        for art in articles:
            articles_html += f"""
            <div class="article">
                <a class="article-title" href="{art['link']}" target="_blank">{art['title']}</a>
                <span class="source">{art['source']}</span>
                <p class="summary">{art['ai_summary']}</p>
            </div>"""
        sections_html += f"""
        <div class="section">
            <h2>{category}</h2>
...
<style>
  ...
  h2 {{ font-size:16px; letter-spacing:1.5px; text-transform:uppercase; color:#8b6914;
        margin:0 0 20px; padding-bottom:8px; border-bottom:2px solid #e8d5b0; }}
  .article {{ margin-bottom:20px; }}
  .article-title {{ font-size:16px; font-weight:bold; color:#1a1a2e; text-decoration:none; line-height:1.3; display:block; }}
  .article-title:hover {{ color:#8b6914; }}
...
<div class="wrapper">
  ...
  <div class="footer">Generated by GitHub Actions · Powered by Groq · Delivered by Resend</div>
</div>
</body>
</html>"""
